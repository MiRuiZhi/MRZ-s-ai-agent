...
这次重构不是逐行翻译旧后端，而是用 Python+C++ 技术栈重建同一类系统：
...
瘦身后的运行时只保留五类核心能力：前端、agent-api、tool-runtime、cpp-worker、持久化服务。旧 Java/Maven 后端源码已从当前工作树删除，历史对照依据保留在 Git 历史中。
...
本轮项目瘦身采用主线收敛策略：删除确定不参与当前 Python+C++ 功能的生成物、旧平台二进制和旧 Java/Maven 后端源码，让工作树只保留可运行主链路代码。
...
`.dockerignore` 额外排除了 assets、runtime、文档、本地缓存、虚拟环境和 `.exe`。Docker 镜像只复制 `services/agent-api`、`services/cpp-worker`、`reactor-tool`、`ui` 和 `deploy/nginx.conf` 等运行相关内容。

## 3. 历史职责到 Python/C++ 的映射

旧 Java/Maven 后端源码已从当前工作树删除，下表只记录迁移时的职责映射。

| 历史职责 | Python/C++ 新位置 | 设计说明 |
| --- | --- | --- |
| HTTP Controller | `services/agent-api/agent_api/api` | FastAPI route |
...
- `FileInformation`：兼容历史前端文件字段，例如 `fileName`、`ossUrl`、`domainUrl`、`fileSize`。
- `AgentMessage`：兼容历史消息、tool_calls、artifact_refs、上传文件。
- `AgentRequest`：新 Python 主入口 `/AutoAgent` 使用。
- `GptQueryRequest`：兼容历史入口 `/web/api/v1/gpt/queryAgentStreamIncr`。
...
| `/web/api/v1/gpt/queryAgentStreamIncr` | POST | 历史兼容主入口 |
...
3. 把历史兼容请求 `GptQueryRequest` 转成内部 `AgentRequest`。
...
历史 Admin 的 DTO/PO 很多，且原仓库缺少完整 SQL。
...
6. 再讲兼容：历史入口、前端 SSE、Admin、文件上传、dataAgent。
7. 再讲部署：Docker Compose、MySQL、Qdrant、nginx、volume。
8. 最后讲风险：dataAgent 未完整迁移、Admin 还是通用配置、生产鉴权要补。

面试可以用一句话总结：

> 我没有逐行翻译旧后端，而是保留原系统的业务协议和 Agent 运行语义，用 Python 重建 API、SSE、Agent 编排和持久化，用 C++ 封装低层执行边界，并复用现有工具运行时，形成一个两服务内聚、可单机 Docker 部署、可继续生产化演进的 Agent 后端。
