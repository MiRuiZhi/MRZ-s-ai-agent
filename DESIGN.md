...
这份文档解释当前 Python+C++ 重构版每个部分的设计。它不是只给一个高层架构图，而是尽量把服务边界、模块职责、数据模型、SSE 协议、Agent 循环、工具调用、C++ worker、部署和测试都拆开讲清楚，方便学习、面试和后续开发。

## 1. 设计目标

原项目是 Java 多模块 Agent 后端，核心价值在于：
...
这次重构不是逐行翻译 Java，而是用 Python+C++ 技术栈重建同一类系统：

- Python 负责 HTTP、SSE、Agent 编排、ORM、业务协议、工具适配。
- C++ 只负责底层执行边界，例如受控命令执行、超时、退出码、stdout/stderr、文件产物扫描、sha256。
- 原有 `reactor-tool` 继续作为工具运行时，不重写已有工具。
- 原有前端 UI 保留，只做协议兼容。
- 数据库使用 MySQL 8，迁移使用 Alembic。
- 向量检索保留 Qdrant。
...
- 保留 `reactor-tool` 能最大化复用已有工具资产。
- 前端不重写能显著降低迁移风险。

## 2. 总体架构

瘦身后的运行时只保留五类核心能力：前端、agent-api、tool-runtime、cpp-worker、持久化服务。Java 旧源码保留在仓库中作为学习参考，但不进入 Docker build context，也不参与运行链路。
...
本轮项目瘦身采用保守策略：只删除确定不参与当前 Python+C++ 功能的生成物和旧平台二进制，避免误删仍有学习价值的 Java 对照源码。

已删除的内容：

- `services/agent-api/.venv`：本地虚拟环境，运行和测试时可以重新创建。
- `services/agent-api/**/__pycache__`、`*.pyc`：Python 编译缓存。
- `.DS_Store`、`assets/.DS_Store`、`.codegraph`：本地系统或索引生成物。
- `reactor-tool/xiaohongshu-login-windows-amd64.exe`
- `reactor-tool/xiaohongshu-mcp-windows-amd64.exe`

后两个 `.exe` 是 Windows-only 小红书 MCP 辅助二进制，不参与当前 Linux Docker 运行时，也不被 `agent-api`、`tool-runtime`、`cpp-worker` 调用。删除它们可以减少仓库体积和 Docker build context 风险。

保留的内容：

- `Reactor-agent-*`：原 Java 源码，保留作学习对照和迁移映射。
- `reactor-tool`：当前工具运行时，必须保留。
- `ui`：当前 React UI，必须保留。
- `services/agent-api`：Python 编排服务，必须保留。
- `services/cpp-worker`：C++ worker，必须保留。

`.dockerignore` 额外排除了 Java 旧模块、assets、runtime、文档、本地缓存、虚拟环境和 `.exe`。这意味着 Java 源码虽然留在仓库中，但不会进入 Docker 构建上下文，也不会影响镜像体积和构建速度。

## 3. Java 到 Python/C++ 的映射

| Java 模块 | Python/C++ 新位置 | 设计说明 |
| --- | --- | --- |
| `Reactor-agent-trigger` | `services/agent-api/agent_api/api` | Controller 变成 FastAPI route |
| `Reactor-agent-case` | `services/agent-api/agent_api/runtime.py` | application service 变成 runtime 编排 |
| `Reactor-agent-domain` | `services/agent-api/agent_api/core` | domain agent、memory、tool、event、ledger 接口 |
| `Reactor-agent-infrastructure` | `services/agent-api/agent_api/storage` | SQLAlchemy model、SQL ledger、Alembic |
| `reactor-tool` | `reactor-tool` | 继续作为工具服务 |
| Java `SseEmitter` | FastAPI `StreamingResponse` | 每帧 `data: JSON` |
| Java 并发 | `asyncio.gather`、`asyncio.Semaphore` | 工具和 PlanSolve 子任务并发 |
| Java PO/DTO/DAO | Pydantic schema + SQLAlchemy model | 请求响应和持久化分离 |
| Java script/runtime 边界 | C++ worker | 低层执行隔离 |
...
- `FileInformation`：兼容 Java/前端文件字段，例如 `fileName`、`ossUrl`、`domainUrl`、`fileSize`。
- `AgentMessage`：兼容历史消息、tool_calls、artifact_refs、上传文件。
- `AgentRequest`：新 Python 主入口 `/AutoAgent` 使用。
- `GptQueryRequest`：兼容 Java 原入口 `/web/api/v1/gpt/queryAgentStreamIncr`。
...
- 保留 Java DTO 语义，但不强行复制 Java 类层次。
...
| `/web/api/v1/gpt/queryAgentStreamIncr` | POST | Java 兼容主入口 |
...
3. 把 Java 兼容请求 `GptQueryRequest` 转成内部 `AgentRequest`。
...
- tool-runtime response 历史格式不完全统一，所以这里做兼容归一。
...
- 在还没有完整复刻所有 Java Admin DTO 前，用通用配置表保证数据不丢。
- 后续可以把高频配置迁移为强类型表，仍保留 `config_record` 做扩展配置。
...
保留它，可以把重构重点放在 Agent 编排和协议兼容上。

### 19.4 为什么 Admin 先用通用配置表

Java Admin 的 DTO/PO 很多，且原仓库缺少完整 SQL。
...
1. 先讲迁移目标：保留 ReAct、PlanSolve、SSE、工具和账本语义。
...
6. 再讲兼容：Java 入口、前端 SSE、Admin、文件上传、dataAgent。
7. 再讲部署：Docker Compose、MySQL、Qdrant、nginx、volume。
8. 最后讲风险：dataAgent 未完整迁移、Admin 还是通用配置、生产鉴权要补。

面试可以用一句话总结：

> 我没有逐行翻译 Java，而是保留原系统的业务协议和 Agent 运行语义，用 Python 重建 API、SSE、Agent 编排和持久化，用 C++ 封装低层执行边界，并复用现有工具运行时，形成一个两服务内聚、可单机 Docker 部署、可继续生产化演进的 Agent 后端。
