...
- 用 FastAPI 替代 Java Controller，提供兼容的 API 和 SSE 输出。
...
仓库里仍然保留 `Reactor-agent-*` Java 模块，因为它有学习和对照价值：

- 可以对照 Java Controller 和 FastAPI route 的迁移方式。
- 可以对照 Java domain/case/infrastructure 分层和 Python api/core/storage 分层。
- 可以对照 Java DTO/PO/DAO 和 Python Pydantic/SQLAlchemy。
- 可以在面试中说明自己不是只写 demo，而是理解了一个已有复杂系统，再做技术栈迁移。

但 Java 源码不进入 Docker build context，也不参与当前运行链路。

这通过 `.dockerignore` 控制：

- Java 保留在仓库。
- Docker 构建不复制 Java 模块。
- 镜像体积和构建速度不受旧源码影响。
